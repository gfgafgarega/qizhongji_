...
void Emm_V5_Modify_Ctrl_Mode(UART_HandleTypeDef *huart, uint8_t addr, uint8_t svF, uint8_t ctrl_mode); // 发送命令修改开环/闭环控制模式
void Emm_V5_En_Control(UART_HandleTypeDef *huart, uint8_t addr, uint8_t state, uint8_t snF); // 电机使能控制
...
void Emm_V5_Stop_Now(UART_HandleTypeDef *huart, uint8_t addr, uint8_t snF); // 让电机立即停止运动
...
void Emm_V5_Origin_Modify_Params(UART_HandleTypeDef *huart, uint8_t addr, uint8_t svF, uint8_t o_mode, uint8_t o_dir, uint16_t o_vel, uint32_t o_tm, uint16_t sl_vel, uint16_t sl_ma, uint16_t sl_ms, uint8_t potF); // 修改回零参数
